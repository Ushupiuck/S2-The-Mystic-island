; ---------------------------------------------------------------------------

CheckLevelForWater:
		tst.b	(Water_flag).w
		beq.w	ChangeWaterSurfacePos.return
		move.w	#$8000+$14,(a6)	; enable h-int
		moveq	#0,d0
		move.b	(Current_Act).w,d0
		add.w	d0,d0
		lea	(WaterHeight).l,a1
		move.w	(a1,d0.w),d0
		move.w	d0,(v_waterpos1).w
		move.w	d0,(v_waterpos2).w
		move.w	d0,(v_waterpos3).w
		sf	(v_wtr_routine).w
		sf	(f_wtr_state).w
		move.b	#1,(Water_flag).w
		move.w	#30,(v_air).w
		moveq	#palid_LZSonWater,d0
		cmpi.b	#103,(Current_ZoneAndAct).w	; Labyrinth act 4?
		bne.s	.loadpalette			; if not, load from lampost
		moveq	#palid_SBZ3SonWat,d0

.loadpalette:	move.b	(v_lamp_wtrstat).w,(f_wtr_state).w
		bra.w	PalLoad3_Water
; ---------------------------------------------------------------------------
; Subroutine to move the water or oil surface sprites to where the screen is at
; (the closest match I could find to this subroutine in Sonic 1 is Obj1B_Action)
; ---------------------------------------------------------------------------

; =============== S U B	R O U T	I N E =======================================


ChangeWaterSurfacePos:
		tst.b	(Water_flag).w
		beq.s	.return
		move.w	(Camera_RAM).w,d1
		move.b	(v_jpadpress1).w,d0 ; is Start button pressed?
		or.b	(v_jpadpress2).w,d0 ; (either player)
		andi.b	#btnStart,d0
		bne.s	+
		btst	#0,(Timer_frames+1).w
		beq.s	+
		addi.w	#$20,d1
+		; match obj x-position to screen position
		move.w	d1,d0
		addi.w	#$60,d0
		move.w	d0,(v_watersurface1+obX).w
		addi.w	#$120,d1
		move.w	d1,(v_watersurface2+obX).w
.return:	rts
; End of function ChangeWaterSurfacePos


; =============== S U B	R O U T	I N E =======================================

; ---------------------------------------------------------------------------
; Subroutine to do special water effects
; ---------------------------------------------------------------------------
; LZWaterEffects:
WaterEffects:
		tst.b	(Water_flag).w	; does level have water?
		beq.s	.return
		tst.b	(Deform_lock).w
		bne.s	.setheight
		cmpi.b	#6,(v_player+obRoutine).w	; is player dead?
		bcc.s	.setheight
		bsr.w	DynamicWaterHeight

.setheight:
		sf.b	(f_wtr_state).w
		moveq	#0,d0
	;	cmpi.b	#aquatic_ruin_zone,(Current_Zone).w
	;	beq.s	+	; This is an example on how not to have oscillating water
		move.b	(v_oscillate+2).w,d0
		lsr.w	#1,d0
+
		add.w	(v_waterpos2).w,d0
		move.w	d0,(v_waterpos1).w
		; calculate distance between water surface and top of screen
		move.w	(v_waterpos1).w,d0
		sub.w	(Camera_Y_pos).w,d0
		bcc.s	.isbelow
		tst.w	d0
		bpl.s	.isbelow
		move.b	#224-1,(v_hbla_line).w
		st.b	(f_wtr_state).w

.isbelow:
		cmpi.w	#224-1,d0
		bcs.s	.isvisible
		move.w	#224-1,d0

.isvisible:	move.b	d0,(v_hbla_line).w
.return:	rts
; End of function WaterEffects
; ===========================================================================
; ---------------------------------------------------------------------------
; Initial water heights
; ---------------------------------------------------------------------------
WaterHeight:	dc.w $600	; Hidden Palace 1
		dc.w $328	; Labyrinth 2
		dc.w $900	; Labyrinth 3
		dc.w $228	; Scrap Brain 3
		even
; ===========================================================================

; ---------------------------------------------------------------------------
; Per-level dynamic water routines
; ---------------------------------------------------------------------------

DynamicWaterHeight:
		moveq	#0,d0
		move.b	(Current_Act).w,d0
		add.w	d0,d0
		move.w	DynWater_Index(pc,d0.w),d0
		jsr	DynWater_Index(pc,d0.w)
		moveq	#0,d1
		move.b	(Water_flag).w,d1
		move.w	(v_waterpos3).w,d0
		sub.w	(v_waterpos2).w,d0
		beq.s	.return
		bcc.s	.movewater
		neg.w	d1

.movewater:	add.w	d1,(v_waterpos2).w
.return:	rts
; End of function DynamicWaterHeight

; ---------------------------------------------------------------------------
DynWater_Index:	; 4 entries per level; 1 per act
;		dc.w DynWater_Null-DynWater_Index	; 0
;		dc.w DynWater_Null-DynWater_Index
;		dc.w DynWater_Null-DynWater_Index
;		dc.w DynWater_Null-DynWater_Index
		dc.w DynWater_Debug-DynWater_Index
		dc.w DynWater_LZ2-DynWater_Index	; leftover from Sonic 1's LZ2
		dc.w DynWater_LZ3-DynWater_Index	; leftover from Sonic 1's LZ3
		dc.w DynWater_LZ4-DynWater_Index	; leftover from Sonic 1's LZ4
;		dc.w DynWater_Null-DynWater_Index	; 1
;		dc.w DynWater_Null-DynWater_Index
;		dc.w DynWater_Null-DynWater_Index
;		dc.w DynWater_Null-DynWater_Index
;		dc.w DynWater_Null-DynWater_Index	; 2
;		dc.w DynWater_Null-DynWater_Index
;		dc.w DynWater_Null-DynWater_Index
;		dc.w DynWater_Null-DynWater_Index
; ---------------------------------------------------------------------------

DynWater_Debug:						; This uses the 2nd controller to make the water level move up or down
		btst	#bitUp,(v_jpadhold2).w		; is UP being held by player 2?
		beq.s	.down				; no? check for down.
		tst.w	(v_waterpos3).w
		beq.s	.down
		subq.w	#1,(v_waterpos3).w

.down:
		btst	#bitDn,(v_jpadhold2).w		; is DOWN being held by player 2?
		beq.s	.return				; no? check for...
		cmpi.w	#$700,(v_waterpos3).w
		beq.s	.return				; we don't go any further down than this
		addq.w	#1,(v_waterpos3).w

.return:	rts	; ...Horizontal water? You wish lol
; ---------------------------------------------------------------------------

DynWater_LZ1:				; leftover from Sonic 1
		move.w	(Camera_RAM).w,d0
		move.b	(v_wtr_routine).w,d2
		bne.s	.routine2
		move.w	#$B8,d1			; water height
		cmpi.w	#$600,d0		; has screen reached next position?
		blo.s	.setwater		; if not, branch
		move.w	#$108,d1
		cmpi.w	#$200,(v_player+obY).w	; is Sonic above $200 y-axis?
		blo.s	.sonicishigh		; if yes, branch
		cmpi.w	#$C00,d0
		blo.s	.setwater
		move.w	#$318,d1
		cmpi.w	#$1080,d0
		blo.s	.setwater
		move.b	#$80,(f_switch+5).w
		move.w	#$5C8,d1
		cmpi.w	#$1380,d0
		blo.s	.setwater
		move.w	#$3A8,d1
		cmp.w	(v_waterpos2).w,d1	; has water reached last height?
		bne.s	.setwater		; if not, branch
		move.b	#1,(v_wtr_routine).w	; use second routine next

.setwater:	move.w	d1,(v_waterpos3).w
		rts
; ---------------------------------------------------------------------------

.sonicishigh:
		cmpi.w	#$C80,d0
		bcs.s	.setwater
		move.w	#$E8,d1
		cmpi.w	#$1500,d0
		bcs.s	.setwater
		move.w	#$108,d1
		move.w	d1,(v_waterpos3).w
		rts
; ---------------------------------------------------------------------------

.routine2:
		subq.b	#1,d2
		bne.s	.skip
		cmpi.w	#$2E0,(v_player+obY).w	; is Sonic above $2E0 y-axis?
		bcc.s	.skip			; if not, branch
		move.w	#$3A8,d1
		cmpi.w	#$1300,d0
		bcs.s	.setwater2
		move.w	#$108,d1
		move.b	#2,(v_wtr_routine).w

.setwater2:	move.w	d1,(v_waterpos3).w
.skip:		rts
; ---------------------------------------------------------------------------

DynWater_LZ2:
		move.w	(Camera_RAM).w,d0		; leftover from Sonic 1's LZ2
		move.w	#$328,d1
		cmpi.w	#$500,d0
		bcs.s	.setwater
		move.w	#$3C8,d1
		cmpi.w	#$B00,d0
		bcs.s	.setwater
		move.w	#$428,d1

.setwater:	move.w	d1,(v_waterpos3).w
		rts
; ---------------------------------------------------------------------------

DynWater_LZ3:	; Leftover from Sonic 1's LZ3
		move.w	(Camera_RAM).w,d0
		move.b	(v_wtr_routine).w,d2
		bne.s	.routine2

		move.w	#$900,d1
		cmpi.w	#$600,d0	; has screen reached position?
		bcs.s	.setwaterlz3	; if not, branch
		cmpi.w	#$3C0,(v_player+obY).w
		bcs.s	.setwaterlz3
		cmpi.w	#$600,(v_player+obY).w	; is Sonic in a y-axis range?
		bcc.s	.setwaterlz3		; if not, branch

		move.w	#$4C8,d1			; set new water height
	;	move.b	#$4B,(v_lvllayout+$80*2+6).w	; update level layout (leftover from Sonic 1. Functional, but Also... Invalid? If restored, we'd have to update a 2x2 grid of 4 chunks, not 1)
		move.b	#1,(v_wtr_routine).w
	;	move.w	#sfx_Rumbling,d0
	;	bsr.w	PlaySound_Special		; play sound $B7 (rumbling)

.setwaterlz3:	move.w	d1,(v_waterpos3).w
		move.w	d1,(v_waterpos2).w
		rts
; ---------------------------------------------------------------------------

.routine2:
		subq.b	#1,d2
		bne.s	.routine3
		move.w	#$4C8,d1
		cmpi.w	#$770,d0
		bcs.s	.setwater2
		move.w	#$308,d1
		cmpi.w	#$1400,d0
		bcs.s	.setwater2
		cmpi.w	#$508,(v_waterpos3).w
		beq.s	.sonicislow
		cmpi.w	#$600,(v_player+obY).w	; is Sonic below $600 y-axis?
		bcc.s	.sonicislow		; if yes, branch
		cmpi.w	#$280,(v_player+obY).w
		bcc.s	.setwater2

.sonicislow:
		move.w	#$508,d1
		move.w	d1,(v_waterpos2).w
		cmpi.w	#$1770,d0
		bcs.s	.setwater2
		move.b	#2,(v_wtr_routine).w

.setwater2:	move.w	d1,(v_waterpos3).w
		rts
; ---------------------------------------------------------------------------

.routine3:
		subq.b	#1,d2
		bne.s	.routine4
		move.w	#$508,d1
		cmpi.w	#$1860,d0
		bcs.s	.setwater3
		move.w	#$188,d1
		cmpi.w	#$1AF0,d0
		bcc.s	+
		cmp.w	(v_waterpos2).w,d1
		bne.s	.setwater3
+
		move.b	#3,(v_wtr_routine).w

.setwater3:	move.w	d1,(v_waterpos3).w
		rts
; ---------------------------------------------------------------------------

.routine4:
		subq.b	#1,d2
		bne.s	.routine5
		move.w	#$188,d1
		cmpi.w	#$1AF0,d0
		bcs.s	.setwater4
		move.w	#$900,d1
		cmpi.w	#$1BC0,d0
		bcs.s	.setwater4
		move.b	#4,(v_wtr_routine).w
		move.w	#$608,(v_waterpos3).w
		move.w	#$7C0,(v_waterpos2).w
		move.b	#1,(f_switch+8).w
		rts
; ---------------------------------------------------------------------------

.setwater4:
		move.w	d1,(v_waterpos3).w
		move.w	d1,(v_waterpos2).w
		rts
; ---------------------------------------------------------------------------

.routine5:
		cmpi.w	#$1E00,d0	; has screen passed final position?
		bcs.s	.dontset	; if not, branch
		move.w	#$128,(v_waterpos3).w
.dontset:	rts
; ---------------------------------------------------------------------------

DynWater_LZ4:
		move.w	#$228,d1			; Leftover from Sonic 1's SBZ3
		cmpi.w	#$F00,(Camera_RAM).w
		bcs.s	.setwater
		move.w	#$4C8,d1

.setwater:	move.w	d1,(v_waterpos3).w
		rts
; ---------------------------------------------------------------------------
; Wind tunnels subroutine
; ---------------------------------------------------------------------------

; ||||||||||||||| S U B R O U T I N E |||||||||||||||||||||||||||||||||||||||


WindTunnels:	; leftover from Sonic 1's LZ
		tst.w	(Debug_placement_mode).w; is debug mode being used?
		bne.w	.return			; if so, bail
		cmpi.b	#1,(Current_Zone).w	; From Sonic 3 & knuckles
		bne.w	.return			; if we're not in labyrinth/hidrocity, bail
	;	cmpi.w	#2,(Player_mode).w	; Tails only game?
	;	beq.s	loc_6F82		; Branch if so
	;	lea	(f_wtunnelallow).l,a3	; Sonic 3 & knuckles manually sets this to 1 later
	;	lea	(f_wtunnelallow_p2).l,a3; For both players, conditionally. TODO
		lea	(WindTunnel_Data).l,a2
		moveq	#0,d0
		move.b	(Current_Act).w,d0	; get act number
		lsl.w	#3,d0			; multiply by 8
		adda.w	d0,a2			; add to address for data
		moveq	#0,d1
		tst.b	(Current_Act).w		; is act number 1?
		beq.s	.notact1		; if so, skip
		lea	(WindTunnel_Data+8).l,a2
		addq.w	#8,a2			; use different data for act 2

.notact1:
		lea	(v_player).w,a1
	;	bsr.s	.chksonic
	;	lea	(v_player2).w,a1

.chksonic:
		move.w	obX(a1),d0
		cmp.w	(a2),d0
		bcs.w	.chknext
		cmp.w	4(a2),d0
		bcc.w	.chknext
		move.w	obY(a1),d2
		cmp.w	2(a2),d2
		bcs.w	.chknext
		cmp.w	6(a2),d2
		bcc.s	.chknext	; branch if Sonic is outside a range
		move.w	d0,d1		; backup d0; it'll be overwritten
		move.b	(Vint_runcount+3).w,d0
		andi.b	#$3F,d0		; does VInt counter fall on 0, $40, $80 or $C0?
		bne.s	.skipsound	; if not, branch
		move.w	#sfx_Waterfall,d0
		jsr	(PlaySound_Special).l	; play rushing water sound (only every $40 frames)

.skipsound:
		tst.b	(f_wtunnelallow).w ; are wind tunnels disabled?
		bne.w	.return		; if yes, branch
		cmpi.b	#4,obRoutine(a1)	; is Sonic hurt/dying?
		bhs.s	.clrquit	; if yes, branch
		move.b	#1,(f_wtunnelmode).w
		move.w	d1,d0		; restore d0
		subi.w	#$80,d0
		cmp.w	(a2),d0
		bcc.s	.movesonic
		moveq	#2,d0
		cmpi.b	#1,(Current_Act).w	; is act number 2?
		bne.s	.notact2		; if not, branch
		neg.w	d0

.notact2:
		add.w	d0,obY(a1)		; adjust Sonic's y-axis for curve of tunnel

.movesonic:
		addi.w	#4,obX(a1)
		move.w	#$400,obVelX(a1)	; move Sonic horizontally
		clr.w	obVelY(a1)
		move.b	#$F,obAnim(a1)		; use floating animation
		bset	#1,obStatus(a1)		; set "in-air" bit
		btst	#0,(v_jpadholdlogical).w	; is up pressed?
		beq.s	.down			; if not, branch
		subq.w	#1,obY(a1)		; move Sonic up on pole

.down:
		btst	#bitDn,(v_jpadhold2).w	; is down being pressed?
		beq.s	.return			; if not, branch
		addq.w	#1,obY(a1)		; move Sonic down on pole
		rts
; ---------------------------------------------------------------------------

.chknext:
		addq.w	#8,a2		; use second set of values (act 1 only)
		dbf	d1,.chksonic	; on act 1, repeat for a second tunnel
		tst.b	(f_wtunnelmode).w ; is Sonic still in a tunnel?
		beq.s	.return		; if yes, branch
		sf	obAnim(a1)	; use walking animation

.clrquit:	clr.b	(f_wtunnelmode).w
.return:	rts
; End of function WindTunnels

; ---------------------------------------------------------------------------
; Wind Tunnel Coordinates (Current boundaries leftover from Sonic 1; Filler)
; ---------------------------------------------------------------------------
WindTunnel_Data:;    left, top,  right, bottom boundaries
		dc.w $A80, $300, $C10,  $380 ; act 1 values (set 1)
		dc.w $F80, $100, $1410,	$180 ; act 1 values (set 2)

		dc.w $460, $400, $710,  $480 ; act 2 values

		dc.w $A20, $600, $1610, $6E0 ; act 3 values

		dc.w $C80, $600, $13D0, $680 ; act 4 values (SBZ3)
		even
; ---------------------------------------------------------------------------
; Labyrinth Zone water slide subroutine
; ---------------------------------------------------------------------------

; ||||||||||||||| S U B R O U T I N E |||||||||||||||||||||||||||||||||||||||


WaterSlides:
		lea	(v_player).w,a1
		btst	#1,obStatus(a1)	; is Sonic jumping?
		bne.s	.character_in_air	; if not, branch
		move.w	obY(a1),d0
		andi.w	#$700,d0
		move.b	obX(a1),d1
		andi.w	#$7F,d1
		add.w	d1,d0
		lea	(v_lvllayout).w,a2
		move.b	(a2,d0.w),d0
		lea	Slide_Chunks_End(pc),a2
		moveq	#Slide_Chunks_End-Slide_Chunks-1,d1

.loop:
		cmp.b	-(a2),d0
		dbeq	d1,.loop
		beq.s	Slide_Move

.character_in_air:
		tst.b	(f_slidemode).w
		beq.s	.return
		move.w	#5,move_lock(a1)	; In NA, move_lock changes from $3E, to $2E.
		clr.b	(f_slidemode).w
.return:	rts
; ---------------------------------------------------------------------------

Slide_Move:
		cmpi.w	#3,d1
		bcc.s	loc_441A
		nop

loc_441A:
		bclr	#0,obStatus(a1)
		move.b	Slide_Speeds(pc,d1.w),d0
		move.b	d0,obInertia(a1)
		bpl.s	loc_4430
		bset	#0,obStatus(a1)

loc_4430:
		clr.b	obInertia+1(a1)
		move.b	#$1B,obAnim(a1)		; use Sonic's "sliding" animation
		move.b	#1,(f_slidemode).w	; set water slide flag
		move.b	(Vint_runcount+3).w,d0
		andi.b	#$1F,d0
		bne.s	WaterSlides.return
		move.w	#sfx_Waterfall,d0
		jmp	(PlaySound_Special).l	; play water sound
; End of function WaterSlides

; ---------------------------------------------------------------------------
Slide_Speeds:	dc.b 10, -11, 10, -10, -11, -12, 11
		even
Slide_Chunks:	dc.b 2, 7, 3, $4C, $4B, 8, 4
Slide_Chunks_End:
		even